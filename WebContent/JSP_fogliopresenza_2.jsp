...
<s:if
    test="%{( #request.foglioPresenze.stato != 'approvato' && #request.foglioPresenze.stato != 'lavorazione')}">
...
<!--    
   Se lo STATO è <> da APPROVATO e LAVORAZIONE  Pulsanti salva e approva******************************    
-->
        ...
        <tr>
            <td><form id="salvaForm" action="salvaFoglio">
                    ...
                    <input type="hidden" name="contratto"
                           ...
                           type="hidden" name="osservazioni_foglio" id="n1"> <input
                           type="hidden" name="note_busta" id="n2"> <input
                           id="salvaButton" type="button" value="SALVA" onclick="save()">
                </form></td>
                <s:if
                    test="%{( #request.foglioPresenze.stato != 'approvato' && #request.foglioPresenze.stato != 'lavorazione')}">
                <td>
                    <form action="aggiornaStatoFoglio">
                        <s:hidden id="accessType" name="accessType" value="%{accessType}"/>
                        <input type="hidden" name="contratto"
                               value="<s:property value='#request.contratto'/>"> <input
                               type="hidden" name="mese"
                               ...
                               type="hidden" name="anno"
                               value="<s:property value='#request.anno'/>"> <input
                               type="hidden" name="stato" value="approvato"> <input
...
<table style="border-bottom-color: gray; border-width: 2px;"
...
http://path_action/updateDay.action?idfogli_presenze=giornate[x].id
            ...
            <s:if
                ...
                <s:iterator value="%{#request.giornate}" var="giornata"
                    ...
                    <th class="dayStyle" width="20px"
                        style="text-align: center; background-color: #666; color: #ffffff;"><a
    ...
    <s:else>
        <s:iterator value="%{#request.giornate}" var="giornata"
                    status="incr">
            <th class="dayStyle" width="20px"
                style="text-align: center; background-color: #666; color: #ffffff;"><s:property
...
<%-- <s:url id="insertValueDay_params_Url" action="insertValueDay" includeContext="false">
                                <s:param name="contratto" value="%{#request.contratto}"> </s:param>
                <s:param name="anno" value="%{#request.anno}"> </s:param>
                <s:param name="mese" value="%{#request.mese}"> </s:param>
</s:url> --%>
        ...
        <table id="dayTable"
            ...
            <thead>
                ...
                <tr>
                    ...
                    <tr>
                        <td><br /> <s:submit  class="myOKButton" value="Inserisci Ore"
...
<!--
            Se lo STATO è diverso da approvato e lavorazione INSERISCI ferie malattie dal/al -------------------------------------------------------------------------------------------------
-->
...
<!-- ------------------copio questa parte sotto ed elimino il readonly, in qualsiasi stato si può modifcare-------------------------------------------------------->
<!--<br />
<table style="width: 20%;   border-bottom-color: gray; border-width: 1px;" border = 0>
    <tr><b>Osservazioni:</b><br/>
<s:if test="%{(#request.foglioPresenze.stato != 'inviato' && #request.foglioPresenze.stato != 'approvato')}">
    <td><textarea id="osservazioni" name="osservazioni" rows="4" cols="60" ><s:if test="%{#request.foglioPresenze.osservazioni != 'null'}"><s:property value="#request.foglioPresenze.osservazioni"/></s:if></textarea></td>
</s:if>
<s:else>
<td><textarea readonly="true" id="osservazioni" name="osservazioni" rows="4" cols="60" ><s:if test="%{#request.foglioPresenze.osservazioni != 'null'}"><s:property value="#request.foglioPresenze.osservazioni"/></s:if></textarea></td>
</s:else>
</td>
</tr>
</table>
...
<br />
<table style="width: 20%; border-bottom-color: gray; border-width: 1px;"
       border=0>
    <tr>
    <b>Osservazioni:</b>
    <br />
...
Link di visualizzazione allegato se esiste 
-->
<!--MOSTRA ALLEGATI ***************************************************************************************-->
<table
    style="width: 60%; color: #ffffff; background-color: #bbb; border-bottom-color: #E8E8E8; border-width: 1px;"
    border=0>
...
<s:if
    ...
    <table
        style="width: 60%; border-bottom-color: #E8E8E8; border-width: 2px;"
        border=1>
        <tr>
...
<table
    ...
    border=0>
    <tr>
        <td>Note <br /> <%--<s:if test="%{(#request.foglioPresenze.stato != 'approvato')}">--%>
            <textarea id="noteBusta" name="noteBusta" rows="4" cols="60"><s:if
                    test="%{#request.foglioPresenze.noteBusta != 'null'}">
                    <s:property value="#request.foglioPresenze.noteBusta" />
                </s:if></textarea> <%--</s:if>--%> <%--<s:else>--%> <!--<textarea readonly="true" id="noteBusta" name="noteBusta" rows="4" cols="60"><s:if test="%{#request.foglioPresenze.noteBusta != 'null'}"><s:property value="#request.foglioPresenze.noteBusta"/></s:if></textarea>-->
                <%--</s:else>--%>
        </td>
        ...
        <td><c:choose>
                ...
                test="%{(#request.foglioPresenze.stato != 'lavorazione' )}">
                <s:form action="upload" method="post" enctype="multipart/form-data">
                    <s:file name="file3" label="Busta Paga " />
                    <s:submit value="Carica" align="right"
                              onclick="saveNote('n13','n23')"></s:submit>
                    <s:hidden id="accessType" name="accessType" value="%{accessType}"/>
                    <s:hidden name="contratto" value="%{#request.contratto}" />
                    <s:hidden name="anno" value="%{#request.anno}" />
                    <s:hidden name="mese" value="%{#request.mese}" />
                    <s:hidden name="osservazioni_foglio" id="n13" />
                    <s:hidden name="note_busta" id="n23" />
                </s:form>
            </s:if></td>
...
<!--<table style="width: 80%; border-bottom-color: gray; border-width: 2px;" border = 0>
    <tr> 
        <td>
            ...
            </br>Per appuntamenti e informazioni chiamare il numero 045/8065572 dalle 09:00 alle 12:00
